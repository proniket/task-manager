...
function EditContent({ taskId }: Props) {
	...
	useEffect(() => {
		const fetchData = async () => {
			try {
				...
				setImportant(taskData.isImportant);
			} catch (error) {
	...
	const handleSubmit = async (e: any) => {
		...
		try {
			const res = await axios.put(`/api/tasks/${currentTaskId}`, task);
		...
		<EditContentStyled onSubmit={handleSubmit} theme={theme}>
			...
			<div className='input-control'>
				...
				<input
					type='text'
					id='title'
					// value={currentTask ? currentTask.title : title}
					defaultValue={title}
					name='title'
			...
			<div className='input-control'>
				...
				<textarea
					...
					id='description'
					rows={4}
					placeholder='e.g, Watch a video about Next.js Auth'></textarea>
			</div>
			<div className='input-control'>
			...
			<div className='input-control toggler'>
				...
				<input
					...
					defaultValue={important.toString()}
					checked={important}
